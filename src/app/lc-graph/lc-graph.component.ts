...
@Component({
  selector: 'app-lc-graph',
  templateUrl: './lc-graph.component.html',
  styleUrls: ['./lc-graph.component.scss']
})
export class LcGraphComponent implements OnInit {
  mainChart:Chart;
  ...
  numSensors:number = DataDisplayComponent.LCSensors.length;
  ...
  constructor(private message:MessengerService) {
    ...
    let tempSensors = DataDisplayComponent.LCSensors;
    this.mainData = [
  ...
  ngOnInit(): void {
    this.mainChart = new Chart("loadChart", {
      type: 'line',
      data: {
    ...
    this.message.toLCGraph.subscribe(
      {
